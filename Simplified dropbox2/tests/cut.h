...
#	define ASSERT_FILE(f, content)                                        \
		do {                                                           \
			if (!cut_File(f, content)) {                           \
				cut_Stop("content of file is not equal",       \
					 __FILE__, __LINE__);                  \
			}                                                      \
		} while (0)
...
#	define CHECK_FILE(f, content)                                         \
		do {                                                           \
			if (!cut_File(f, content)) {                           \
				cut_Check("content of file is not equal",      \
					  __FILE__, __LINE__);                 \
			}                                                      \
		} while (0)
...
#	define TEST(name)                                                     \
		...
		{                                                              \
			cut_Register(cut_instance_##name, #name, __FILE__,     \
				     __LINE__);                                \
		}                                                              \
...
void cut_Register(cut_Instance instance, const char *name, const char *file,
		  size_t line);
...
#			define CUT_FRAGMENT_RECEIVE_STATUS                    \
				{                                              \
					0                                      \
				}
...
CUT_PRIVATE void *cut_FragmentReserve(struct cut_Fragment *fragments,
				      size_t length, int *sliceId)
{
	if (!fragments)
		return NULL;
	if (fragments->sliceCount == CUT_MAX_SLICE_COUNT)
		return NULL;
	struct cut_FragmentSlice *slice = (struct cut_FragmentSlice *)malloc(
		sizeof(struct cut_FragmentSlice));
	if (!slice)
		return NULL;
...
CUT_PRIVATE void *cut_FragmentAddString(struct cut_Fragment *fragments,
					const char *str)
{
	if (!fragments)
		return NULL;
...
CUT_PRIVATE char *cut_FragmentGet(struct cut_Fragment *fragments, int sliceId,
				  size_t *length)
{
	if (!fragments)
		return NULL;
...
CUT_PRIVATE int cut_FragmentSerialize(struct cut_Fragment *fragments)
{
	...
	for (struct cut_FragmentSlice *current = fragments->slices; current;
	     current = current->next) {
		length += current->length;
	}
	...
	struct cut_FragmentHeader *header =
		(struct cut_FragmentHeader *)fragments->serialized;
	...
	for (struct cut_FragmentSlice *current = fragments->slices; current;
		...
		*sliceLength = current->length;
		++sliceLength;
		memcpy(fragments->serialized + contentOffset, current->data,
		       current->length);
...
CUT_PRIVATE int cut_FragmentDeserialize(struct cut_Fragment *fragments)
{
	...
	for (uint16_t slice = 0; slice < header->sliceCount;
	     ++slice, ++sliceLength) {
		void *data = cut_FragmentReserve(fragments, *sliceLength, NULL);
		if (!data)
			return 0;
		memcpy(data, fragments->serialized + contentOffset,
		       *sliceLength);
...
CUT_PRIVATE int64_t cut_FragmentReceiveContinue(
	cut_FragmentReceiveStatus *status, void *data, int64_t length)
{
	if (!status)
		return 0;
	if (!data) {
	...
	if (!status->structured.length) {
		...
		status->structured.length =
			((struct cut_FragmentHeader *)data)->length;
	}
...
CUT_PRIVATE size_t
cut_FragmentReceiveProcessed(cut_FragmentReceiveStatus *status)
{
	if (!status)
		return 0;
...
CUT_PRIVATE int cut_SetSubtestName(struct cut_UnitResult *result, int number,
				   const char *name);
CUT_PRIVATE int cut_AddInfo(struct cut_Info **info, size_t line,
			    const char *file, const char *text);
CUT_PRIVATE int cut_SetFailResult(struct cut_UnitResult *result, size_t line,
				  const char *file, const char *text);
...
CUT_PRIVATE const char *cut_GetStatus(const struct cut_UnitResult *result,
				      enum cut_Colors *color);
CUT_PRIVATE const char *cut_ShortPath(const char *path);
CUT_PRIVATE void cut_PrintResult(int base, int subtest, int subtests,
				 const struct cut_UnitResult *result);
...
CUT_PRIVATE void cut_RunUnitForkless(int testId, int subtest,
...
CUT_PRIVATE int cut_SendMessage(const struct cut_Fragment *message)
{
	...
	int64_t r;
	while (remaining &&
	       (r = cut_Write(cut_pipeWrite, message->serialized + position,
			      remaining)) > 0) {
		position += (size_t)r;
		remaining -= (size_t)r;
	}
...
CUT_PRIVATE int cut_ReadMessage(struct cut_Fragment *message)
{
	...
	int64_t toRead = 0;
	size_t processed = 0;
	while ((toRead = cut_FragmentReceiveContinue(
			&status, message->serialized, r)) > 0) {
		processed = cut_FragmentReceiveProcessed(&status);

		if (message->serializedLength < processed + toRead) {
			message->serializedLength =
				(uint32_t)(processed + toRead);
			message->serialized = (char *)realloc(
				message->serialized, message->serializedLength);
			if (!message->serialized)
				cut_FatalExit(
					"cannot allocate memory for reading a message");
		}
		r = cut_Read(cut_pipeRead, message->serialized + processed,
			     (size_t)toRead);
	}
	processed = cut_FragmentReceiveProcessed(&status);
...
CUT_PRIVATE int cut_SendLocalMessage(struct cut_Fragment *message)
{
	...
	if (message->serializedLength + cut_localMessageSize >
	    CUT_MAX_LOCAL_MESSAGE_LENGTH)
		return 0;
	memcpy(cut_localMessage + cut_localMessageSize, message->serialized,
	       message->serializedLength);
...
CUT_PRIVATE void cut_SendOK(int counter)
{
	...
	*pCounter = counter;
	...
	cut_FragmentSerialize(&message) ||
		cut_FatalExit("cannot serialize ok:fragment");
	cut_SendLocalMessage(&message) ||
		cut_FatalExit("cannot send ok:message");
...
void cut_DebugMessage(const char *file, size_t line, const char *fmt, ...)
{
	...
	(buffer = (char *)malloc(length)) ||
		cut_FatalExit("cannot allocate buffer");
	va_end(args1);
	...
	size_t *pLine =
		(size_t *)cut_FragmentReserve(&message, sizeof(size_t), NULL);
	if (!pLine)
		cut_FatalExit("cannot insert debug:fragment:line");
	*pLine = line;
	cut_FragmentAddString(&message, file) ||
		cut_FatalExit("cannot insert debug:fragment:file");
	cut_FragmentAddString(&message, buffer) ||
		cut_FatalExit("cannot insert debug:fragment:buffer");
	cut_FragmentSerialize(&message) ||
		cut_FatalExit("cannot serialize debug:fragment");

	cut_SendLocalMessage(&message) ||
		cut_FatalExit("cannot send debug:message");
	cut_FragmentClean(&message);
	free(buffer);
}
...
void cut_Stop(const char *text, const char *file, size_t line)
{
	...
	*pLine = line;
	cut_FragmentAddString(&message, file) ||
		cut_FatalExit("cannot insert stop:fragment:file");
	cut_FragmentAddString(&message, text) ||
		cut_FatalExit("cannot insert stop:fragment:text");
	cut_FragmentSerialize(&message) ||
		cut_FatalExit("cannot serialize stop:fragment");

	cut_SendLocalMessage(&message) ||
		cut_FatalExit("cannot send stop:message");
	cut_FragmentClean(&message);
...
void cut_Check(const char *text, const char *file, size_t line)
{
	...
	*pLine = line;
	cut_FragmentAddString(&message, file) ||
		cut_FatalExit("cannot insert check:fragment:file");
	cut_FragmentAddString(&message, text) ||
		cut_FatalExit("cannot insert check:fragment:text");
	cut_FragmentSerialize(&message) ||
		cut_FatalExit("cannot serialize check:fragment");

	cut_SendLocalMessage(&message) ||
		cut_FatalExit("cannot send check:message");
	cut_FragmentClean(&message);
}
...
CUT_PRIVATE void cut_StopException(const char *type, const char *text)
{
	...
	cut_FragmentAddString(&message, type) ||
		cut_FatalExit("cannot insert exception:fragment:type");
	cut_FragmentAddString(&message, text) ||
		cut_FatalExit("cannot insert exception:fragment:text");
	cut_FragmentSerialize(&message) ||
		cut_FatalExit("cannot serialize exception:fragment");

	cut_SendLocalMessage(&message) ||
		cut_FatalExit("cannot send exception:message");
	cut_FragmentClean(&message);
}
...
CUT_PRIVATE void cut_Timeouted()
{
	...
	cut_FragmentSignalSafeSerialize(&message) ||
		cut_FatalExit("cannot serialize timeout:fragment");
	cut_SendLocalMessage(&message) ||
		cut_FatalExit("cannot send timeout:message");
}
...
void cut_Subtest(int number, const char *name)
{
	...
	*pNumber = number;
	cut_FragmentAddString(&message, name) ||
		cut_FatalExit("cannot insert subtest:fragment:name");
	cut_FragmentSerialize(&message) ||
		cut_FatalExit("cannot serialize subtest:fragment");

	cut_SendLocalMessage(&message) ||
		cut_FatalExit("cannot send subtest:message");
	cut_FragmentClean(&message);
}
...
CUT_PRIVATE int cut_ReadLocalMessage(struct cut_Fragment *message)
{
	...
	while ((toRead = cut_FragmentReceiveContinue(
		...
		memcpy(message->serialized + processed, cut_localMessageCursor,
		       (size_t)toRead);
...
CUT_PRIVATE void *cut_PipeReader(struct cut_UnitResult *result)
{
	...
	do {
		...
		cut_ReadLocalMessage(&message) ||
			cut_FatalExit("cannot read message");
		cut_FragmentDeserialize(&message) ||
			cut_FatalExit("cannot deserialize message");

		switch (message.id) {
		case cut_MESSAGE_SUBTEST:
			message.sliceCount == 2 ||
				cut_FatalExit("invalid debug:message format");
			cut_SetSubtestName(
				result,
		...
		case cut_MESSAGE_DEBUG:
			message.sliceCount == 3 ||
				cut_FatalExit("invalid debug:message format");
			cut_AddInfo(&result->debug,
				    *(size_t *)cut_FragmentGet(&message, 0,
							       NULL),
				    cut_FragmentGet(&message, 1, NULL),
		...
		case cut_MESSAGE_OK:
			message.sliceCount == 1 ||
				cut_FatalExit("invalid ok:message format");
			result->subtests =
				*(int *)cut_FragmentGet(&message, 0, NULL);
			break;
		case cut_MESSAGE_FAIL:
			message.sliceCount == 3 ||
				cut_FatalExit("invalid fail:message format");
			cut_SetFailResult(result,
					  *(size_t *)cut_FragmentGet(&message,
								     0, NULL),
					  cut_FragmentGet(&message, 1, NULL),
					  cut_FragmentGet(&message, 2, NULL)) ||
		...
		case cut_MESSAGE_EXCEPTION:
			message.sliceCount == 2 ||
				cut_FatalExit(
					"invalid exception:message format");
			cut_SetExceptionResult(
				result, cut_FragmentGet(&message, 0, NULL),
				cut_FragmentGet(&message, 1, NULL)) ||
		...
		case cut_MESSAGE_CHECK:
			message.sliceCount == 3 ||
				cut_FatalExit("invalid check:message format");
			cut_AddInfo(&result->check,
				    *(size_t *)cut_FragmentGet(&message, 0,
							       NULL),
				    cut_FragmentGet(&message, 1, NULL),
				    cut_FragmentGet(&message, 2, NULL)) ||
...
CUT_PRIVATE int cut_AddInfo(struct cut_Info **info, size_t line,
			    const char *file, const char *text)
{
	struct cut_Info *item =
		(struct cut_Info *)malloc(sizeof(struct cut_Info));
	if (!item)
		return 0;
...
CUT_PRIVATE void cut_ExceptionBypass(int testId, int subtest)
{
	...
	try {
		...
		cut_StopException(name.c_str(),
				  e.what() ? e.what() : "(no reason)");
	} catch (...) {
...
CUT_PRIVATE const char *cut_ShortPath(const char *path)
{
	...
	if (cut_arguments.shortPath < 0 ||
	    pathLength <= cut_arguments.shortPath)
		return path;
				 ...
				 const struct cut_UnitResult *result)
{
	...
	if (result->name && subtest) {
		if (result->number <= 1)
			lastPosition -= fprintf(cut_output, "%s%s", indent,
						result->name);
		else {
		...
		if (result->number)
			lastPosition -=
				fprintf(cut_output, " #%d", result->number);
		indent = longIndent;
	} else {
		lastPosition -= base;
	}
	...
	putc('\n', cut_output);
	if (result->failed) {
		for (const struct cut_Info *current = result->check; current;
		     current = current->next) {
			fprintf(cut_output, "%scheck '%s' (%s:%d)\n", indent,
				current->message, cut_ShortPath(current->file),
				current->line);
		}
		if (result->timeouted)
	...
	for (const struct cut_Info *current = result->debug; current;
	     current = current->next) {
...
CUT_PRIVATE int cut_Runner(int argc, char **argv)
{
	...
	qsort(cut_unitTests.tests, cut_unitTests.size,
	      sizeof(struct cut_UnitTest), cut_TestComparator);
	...
	if (cut_arguments.output) {
		...
		if (!cut_output)
			cut_ErrorExit("cannot open file %s for writing",
				      cut_arguments.output);
	}
	...
	for (int i = 0; i < cut_unitTests.size; ++i) {
		if (cut_SkipUnit(i))
			continue;
		++executed;
		int base = fprintf(cut_output, "[%3i] %s", executed,
		...
		for (int subtest = 0; subtest <= subtests; ++subtest) {
			if (cut_arguments.subtestId >= 0 &&
			    cut_arguments.subtestId != subtest)
				continue;
			struct cut_UnitResult result;
		...
		if (subtests > 1) {
			base = fprintf(cut_output, "[%3i] %s (overall)",
				       executed, cut_unitTests.tests[i].name);
			struct cut_UnitResult result;
			memset(&result, 0, sizeof(result));
	...
	fprintf(cut_output,
		...
		"  skipped:   %3i\n"
		"  failed:    %3i\n",
		cut_unitTests.size, executed - failed,
		cut_unitTests.size - executed, failed);
	if (cut_arguments.output)
		fclose(cut_output);
cleanup:
			     ...
			     struct cut_UnitResult *result)
{
	...
	if (!pid) {
		...
		close(cut_pipeWrite) != -1 ||
			cut_FatalExit("cannot close file");
		free(cut_unitTests.tests);
		free(cut_arguments.match);
		if (cut_arguments.output)
			fclose(cut_output);
...
{
	cut_outputsRedirected = 1;
	cut_CreateTemporaryFile(&cut_stdout) ||
		cut_FatalExit("cannot open temporary file");
	cut_CreateTemporaryFile(&cut_stderr) ||
		cut_FatalExit("cannot open temporary file");
...
CUT_PRIVATE void NTAPI cut_TimerCallback(CUT_UNUSED(void *param),
					 CUT_UNUSED(BOOLEAN timerEvent))
{
	cut_Timeouted();
...
CUT_PRIVATE int cut_PreRun()
{
	if (!cut_arguments.noFork && cut_arguments.testId < 0) {
		...
		SetInformationJobObject(cut_jobGroup,
	...
	if (cut_arguments.timeout) {
		CreateTimerQueueTimer(&timer, NULL, cut_TimerCallback, NULL,
				      cut_arguments.timeout * 1000, 0,
				      WT_EXECUTEONLYONCE) ||
			cut_FatalExit("cannot create timer");
	}
	...
	if (timer) {
		DeleteTimerQueueTimer(NULL, timer, NULL) ||
			cut_FatalExit("cannot delete timer");
	}
			     ...
			     struct cut_UnitResult *result)
{
	...
	int length = snprintf(NULL, 0, fmtString, cut_arguments.selfName,
			      testId, subtest, cut_arguments.timeout);
	char *command = (char *)malloc(length + 1);
	sprintf(command, fmtString, cut_arguments.selfName, testId, subtest,
		cut_arguments.timeout);

	CreateProcessA(cut_arguments.selfName, command, NULL, NULL, TRUE,
		       CREATE_BREAKAWAY_FROM_JOB | CREATE_SUSPENDED, NULL, NULL,
		       &startInfo, &procInfo) ||
		cut_FatalExit("cannot create process");
	free(command);

	AssignProcessToJobObject(cut_jobGroup, procInfo.hProcess) ||
		cut_FatalExit("cannot assign process to job object");
	ResumeThread(procInfo.hThread) == 1 ||
	...
	WaitForSingleObject(procInfo.hProcess, INFINITE) == WAIT_OBJECT_0 ||
		cut_FatalExit("cannot wait for single object");
	DWORD childResult;
	GetExitCodeProcess(procInfo.hProcess, &childResult) ||
		cut_FatalExit("cannot get exit code");
...
CUT_NORETURN int cut_FatalExit(const char *reason)
{
	if (cut_outputsRedirected) {
		...
		if (log) {
			fprintf(log,
				"CUT internal error during unit test: %s\n",
				reason);
			fclose(log);
		}
	} else {
		  ...
		  size_t line)
{
	if (cut_unitTests.size == cut_unitTests.capacity) {
		...
		cut_unitTests.tests = (struct cut_UnitTest *)realloc(
			cut_unitTests.tests,
...
CUT_PRIVATE void cut_ParseArguments(int argc, char **argv)
{
	...
	for (int i = 1; i < argc; ++i) {
		...
		if (!strcmp(timeout, argv[i])) {
			++i;
			if (i >= argc ||
			    !sscanf(argv[i], "%u", &cut_arguments.timeout))
				cut_ErrorExit(
					"option %s requires numeric argument",
					timeout);
			continue;
		}
		...
		if (!strcmp(output, argv[i])) {
			...
			if (i < argc) {
				...
				cut_ErrorExit(
					"option %s requires string argument",
					output);
			}
			continue;
		}
		if (!strcmp(exactTest, argv[i])) {
			++i;
			if (i >= argc ||
			    !sscanf(argv[i], "%d", &cut_arguments.testId))
				cut_ErrorExit(
					"option %s requires numeric argument %d %d",
					exactTest, i, argc);
			continue;
		}
		if (!strcmp(subtest, argv[i])) {
			++i;
			if (i >= argc ||
			    !sscanf(argv[i], "%d", &cut_arguments.subtestId))
				cut_ErrorExit(
					"option %s requires numeric argument",
					subtest);
			continue;
		...
		if (!strcmp(shortPath, argv[i])) {
			++i;
			if (i >= argc ||
			    !sscanf(argv[i], "%d", &cut_arguments.shortPath))
				cut_ErrorExit(
					"option %s requires numeric argument",
					shortPath);
			continue;
	...
	if (!cut_arguments.matchSize)
		return;
	cut_arguments.match =
		(char **)malloc(cut_arguments.matchSize * sizeof(char *));
	if (!cut_arguments.match)
		cut_ErrorExit(
			"cannot allocate memory for list of selected tests");
	int index = 0;
	for (int i = 1; i < argc; ++i) {
		if (!strncmp(argv[i], "--", 2)) {
			if (!strcmp(timeout, argv[i]) ||
			    !strcmp(output, argv[i]) ||
			    !strcmp(subtest, argv[i]) ||
			    !strcmp(exactTest, argv[i]) ||
